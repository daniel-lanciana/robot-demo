...
* Unit tests anticipate the use of a global config and non-predetermined board size (extensible)
...
* Monkey patch Array object to support infinite LinkedList next/prev functionality for cleaner robot rotation (no switch statements). Old implementation purposely left in as alternative.

== TODO

* Unit tests failing to load AppConfig (doesn't run app_config and if forced Rails is unititalised). Did not happen with RSpec. Using class variable instead for the time being...
* Refactor Robot move method?
