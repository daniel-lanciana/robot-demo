...
* Monkey patch Array object to support infinite LinkedList next/prev functionality for cleaner robot rotation (no switch statements). Showing off a little -- must be used carefully!
...
* Support for dynamic board size
* Basic thread-safety through a global class vairable map of RobotAdapter objects based on session ID
* Cleaned up dead code/comments auto-generated with a new Rails project
* Skeleton for adding multiple robots with default ID field
* Web interface buttons use new friendly AJAX endpoints. Robot now moved by JavaScript. No JS unit tests...
* Chaos robot!
* Monkey patch JavaScript array class since already did it in Ruby (see: showing off)

== Missing

* Multiple robots on the table at once (although basic foundations of ID are in place)
* Functional tests on the view
